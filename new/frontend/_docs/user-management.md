...
- there will be a HRM portal
- once a new employee is onboarded in the HRM with proper designation.

  - KPI's and KRA's

- The HRM portal ie., `peopleworks` will notify our internal app about the onboard event for which we trigger an email.
- sending to that specific user, with system generated 1 time password, which he can reset on login.
