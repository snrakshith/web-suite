...
- nested functions have access to variables in there `own scope` as well as `outer scope`
...
- Currying is a process in functional programming in which we transform a function with multiple arguments into a sequence of nested functions that take 1 argument at a time

> function f(a,b,c) is transformed to f(a)(b)(c)

- Remember Currying doest not call a function, its simply transforms it.

- Usefull for composing reusable functions
...
- the way we invoke `sayMyName function` in this case is using dot notation
...
- in js we can also invoke a function using `new` keyword
...
- usecases:
  - used for sharing properties & methods across instances
  - is inheritances
...
- Generators are special class of functions that simplify the task of writting Iterators.
- Normal functions follow `run to completion model`
- Generator functions `can pause the execution in middle`
  - `yield` is a keyword that pauses the generator function
  - We don't say generator function returns the value, instead we say it `yields the value`.
  - When we call a generator function we always get a `generator object` & this generator object is an `iterator`
  - Now we can use a `for of loop`

> generator function is a `iterable` from which we get a `iterator`, so finally we can use `for of loop`
