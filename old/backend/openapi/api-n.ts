...
import type { paths, operation } from "./api/v1.ts"; // generated by openapi-typescript
...
// Dynamically import the paths object from the generated API typings
(async () => {
  try {
    // Import the paths object from the generated API typings

    // Define a function to dynamically generate client functions
    function generateClientFunctions() {
      ...
      for (const path in paths) {
        ...
        for (const method in methods) {
          const methodName = `${method.toLowerCase()}_${path.replace(
            /\//g,
            "_"
          )}`;
          const operation: operation = methods[method];
          ...
          client[methodName] = async (params: Record<string, any>) => {
            ...
            try {
              const { response, data, error } = await client[method](
                path,
                options
              );
    ...
    // Example usage
    const { response, data, error } = await client.get_crm_companies({
      query: { cursor: "" },
      headers: {
        "x-apideck-app-id": "",
